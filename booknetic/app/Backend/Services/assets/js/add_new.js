	...
	$(document).ready(function()
	{
					...
					<div class="modal-body new_extra_modal_body">
						...
						<div class="form-row">
							...
							<div class="form-group col-md-3">
								...
								<i class="fa fa-info-circle help-icon do_tooltip" data-content="${booknetic.__('default_zero_means_there_is_no_minimum_requirment')}" data-original-title="" title=""></i>
		...
		}).on( 'select2:select', '#input_extra_category', function( e )
		{
			if ( ! e.params.data.hasOwnProperty( 'element' ) )
			{
				...
				{
					var newOption = new Option(e.params.data.text, result.id, true, true);
		...
		}).on('click', '.confirm_employee_selection', function()
		{
		...
		const registerExtraModalEvents = function () {
			$('.new_extra_modal_body').on( 'select2:select', '#input_extra_category', function( e )
			{
				if ( ! e.params.data.hasOwnProperty( 'element' ) )
				{
					booknetic.ajax( 'add_new_extra_category', { name: e.params.data.text }, function( result )
