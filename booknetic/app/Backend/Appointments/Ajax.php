...
class Ajax extends \BookneticApp\Providers\Core\Controller
{
	...
	public function get_services()
	{
		...
		{
			$addFilter .= ' AND ( SELECT `service_id` FROM '. DB::table( 'service_staff' ) .' WHERE `service_id` = `service`.`id` AND `staff_id` IN ( '.implode( ', ', Permission::myStaffId() ).' ) GROUP BY `service_id` ) IS NOT NULL ';
		}
